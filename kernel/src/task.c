...
void kmt_spin_init(struct spinlock *lk, char *name) {
	lk->locked=0;
	lk->name=name;
}
void kmt_spin_lock(struct spinlock *lk) {
	cli();//disable interrupts
	while(_atomic_xchg(&lk->locked,1)!=0);
	lk->hcpu=_cpu();
	cpu_cnt[lk->hcpu]++;
}
void kmt_spin_unlock(struct spinlock *lk) {
...
//sem started
void kmt_sem_init(sem_t *sem, char *name,int value){
